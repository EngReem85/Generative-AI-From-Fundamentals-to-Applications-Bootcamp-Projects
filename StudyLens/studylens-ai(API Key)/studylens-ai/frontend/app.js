(function () {
  const state = {
    imageBase64: null,
    imageMimeType: null,
    history: [], // Gemini-format contents: [{role:'user'|'model', parts:[...]}]
    busy: false,
  };
  ...
  async function analyzeImage() {
    ...
    els.previewSub.textContent = "نقرأ محتوى الصفحة ونجهّز الشرح...";
    ...
    try {
      ...
      state.history = [
        ...
        { role: "model", parts: [{ text: payload.raw_text }] },
      ];
  ...
  async function sendChat(text) {
    ...
    const typingEl = appendMessage("assistant typing", "يكتب الآن...");

    try {
      const payload = await postJson("/api/chat", { history: state.history });
      state.history.push({ role: "model", parts: [{ text: payload.text }] });
